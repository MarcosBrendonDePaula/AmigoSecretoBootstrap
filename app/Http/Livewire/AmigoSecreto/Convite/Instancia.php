...
class Instancia extends Component
{
    ...
    public function mount($id=-1){
        $this->instancia = new Convite();
        $this->_id = $id;
        $this->novaInstancia = true;
        if($id >= 0){
            $this->novaInstancia = false;
            $this->instancia = Convite::find($id);
        }
    }
    ...
    public function save(){
        ...
        $this->instancia->save();
        $this->emit('amigo.secreto.convite.Update');
    }


    public function render()
    {
        return view('livewire.amigo-secreto.convite.instancia');
    }
}
